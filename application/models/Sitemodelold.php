...
class Sitemodel extends CI_Model {
    ...
    */
    ...
    public function all( $forUserID = '' ) {
    	...
    	$this->db->from('sites');
    	$this->db->where('sites_trashed', 0 AND 'sites_id' > 9); //sites_id < 9 reserved for templates
    ...
    */
    ...
    public function update($siteData, $pages) {
        ...
        if($siteData['sites_id']<10){
            $this->db->where('sites_id', $siteData['sites_id']);
            $this->db->update('sites', array('sites_trashed' => 0));
        }
    ...
    public function getSite($siteID) {
    	...
    	foreach( $folderContent as $key => $item ) {
    		...
    		if( is_array($item) ) {
    			...
    			array_push($assetFolders, rtrim($key,'\/'));
    ...
    */
    ...
    public function getAssetsAndPages( $siteID ) {
    	...
    	$folderContent = directory_map($this->config->item('elements_dir'), 2); //get the first level folders inside $this->config->item('elements_dir')
